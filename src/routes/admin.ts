...
import { getEmailSettings, saveEmailSettings, sendTestEmail, sendTipsterApplicationEmail, sendUserAccountStatusEmail, verifyEmailConfiguration } from '../services/emailService';
...
const predictionBody = z.object({ tipsterId:id, matchId:id.optional().or(z.literal("")), fixture:z.string().min(1).max(200), league:z.string().max(120).optional().or(z.literal("")), prediction:z.string().min(1).max(200), odds:z.coerce.number().min(1), isPremium:z.coerce.boolean().optional(), confidence:z.coerce.number().min(0).max(100).optional().or(z.literal("")), analysis:z.string().max(3000).optional().or(z.literal("")), status:z.enum(["pending","published","won","lost","void"]).optional(), profit:z.coerce.number().optional().or(z.literal("")), systemGenerated:z.coerce.boolean().optional(), source:z.string().optional(), horizon:z.enum(['daily','weekly']).optional() });
...
adminRouter.patch("/users/:id", async(req,res,next)=>{ try { const body=z.object({name:z.string().min(2).max(100).optional(),role:z.enum(["user","admin","tipster"]).optional(),status:z.enum(["active","suspended"]).optional()}).parse(req.body); const user=await User.findById(req.params.id).select("-passwordHash"); if(!user)return res.status(404).json({message:"User not found"}); const previous={role:user.role,status:user.status}; Object.assign(user,body); await user.save(); const shouldEmail=(body.role&&body.role!==previous.role)||(body.status&&body.status!==previous.status); const accountUpdateSent=shouldEmail?await sendUserAccountStatusEmail(user.email,user.name,user.role,user.status):undefined; res.json({data:user,email:shouldEmail?{accountUpdateSent}:undefined}); } catch(e){next(e);} });
...
adminRouter.patch("/tipster-applications/:id/review", async(req,res)=>{ const {action,adminNotes}=reviewSchema.parse(req.body); const application=await TipsterApplication.findById(req.params.id); if(!application)return res.status(404).json({message:"Application not found"}); const auth=req as AuthRequest; const now=new Date(); const map:any={under_review:"under_review",reject:"rejected",more_info:"more_info",suspend:"suspended",approve:"approved"}; application.status=map[action]; application.adminNotes=adminNotes; application.reviewedBy=auth.user!.id as any; application.reviewedAt=now; await application.save();
  ...
  const applicant=await User.findById(application.userId).select('email name'); const reviewEmailSent=applicant?await sendTipsterApplicationEmail(applicant.email,applicant.name,application.status,adminNotes):false; res.json({data:application,email:{tipsterApplicationReviewSent:reviewEmailSent}}); });
...
adminRouter.post('/email-settings/test',async(req,res,next)=>{try{const to=z.object({to:z.string().email()}).parse(req.body).to;const check=await verifyEmailConfiguration();if(!check.ok)return res.status(400).json({message:check.reason});await sendTestEmail(to);res.json({message:'Test email sent'});}catch(e){const message=e instanceof Error?e.message:String(e);return res.status(502).json({message:`Cloudflare Email API rejected the email: ${message}`});}});
