...
billingRouter.get('/webhook-health',requireAuth,async(_req,res,next)=>{try{
  const [lastProcessed,lastReceived,pending]=await Promise.all([
    StripeEvent.findOne({processedAt:{$exists:true}}).sort({processedAt:-1}).select('eventId type processedAt receivedAt'),
    StripeEvent.findOne().sort({receivedAt:-1}).select('eventId type processedAt receivedAt processingAt'),
    StripeEvent.countDocuments({processedAt:{$exists:false},processingAt:{$exists:true}})
  ]);
  res.json({data:{configured:Boolean(env.STRIPE_SECRET_KEY&&env.STRIPE_WEBHOOK_SECRET),endpoint:'/api/v1/billing/webhook',lastProcessed,lastReceived,pendingProcessingEvents:pending}});
}catch(e){next(e)}});
...
export async function stripeWebhook(req:any,res:any){
  try{
    ...
    if(result.busy) return res.status(409).json({received:false,retry:true,message:'Webhook event is currently being processed.'});
    return res.status(200).json({received:true,...result});
  }catch(e:any){
    const message=e?.message||'Webhook error';
    const signatureError=/signature|raw request body|webhook secret|expired/i.test(message);
    return res.status(signatureError?400:500).json({message});
  }
}
