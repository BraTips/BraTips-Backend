...
publicRouter.get('/bet-of-day/recent',async(req,res,next)=>{try{const limit=Math.min(Math.max(Number(req.query.limit)||12,1),50);const data=await BetOfDay.find({status:{ $in:['won','lost','void']}}).sort({updatedAt:-1,createdAt:-1}).limit(limit).populate({path:'matchId',populate:[{path:'homeTeamId',select:'name shortName logo'},{path:'awayTeamId',select:'name shortName logo'},{path:'leagueId',select:'name logo'}]});res.json({data});}catch(e){next(e)}});
