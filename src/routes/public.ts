...
publicRouter.get("/bet-of-day/recent",async(req,res,next)=>{try{const limit=Math.min(Math.max(Number(req.query.limit)||50,1),100);const data=await BetOfDay.find({status:{ $in:["published","won","lost","void"]}}).populate({path:'matchId',populate:[{path:'homeTeamId',select:'name logo'},{path:'awayTeamId',select:'name logo'},{path:'leagueId',select:'name logo'}]}).sort({date:-1,createdAt:-1}).limit(limit);res.json({data});}catch(e){next(e)}});
