...
async function openAISelect(candidates:any[]) {
  ...
  const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model,input:[{role:'system',content:`Select exactly 2 upcoming football matches for an admin Bet of the Day. Do not call anything guaranteed or safe. Prefer conservative markets such as Double Chance, Under/Over when supported. For Double Chance you MUST specify exactly one selection: Double Chance 1X, Double Chance X2, or Double Chance 12. Never return just Double Chance. Return strict JSON array with matchId, prediction, confidence, risk, analysis.`},{role:'user',content:JSON.stringify(candidates)}],text:{format:{type:'json_schema',name:'bet_of_day',schema:{type:'object',properties:{picks:{type:'array',items:{type:'object',properties:{matchId:{type:'string'},prediction:{type:'string'},confidence:{type:'number'},risk:{type:'string'},analysis:{type:'string'}},required:['matchId','prediction','confidence','risk','analysis'],additionalProperties:false},minItems:2,maxItems:2}},required:['picks'],additionalProperties:false},strict:true}}})});
