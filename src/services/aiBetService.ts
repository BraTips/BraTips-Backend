...
export async function generateBetOfDay(date=new Date().toISOString().slice(0,10)){
  ...
  if(!picks){ picks=matches.slice(0,2).map((m:any,i)=>({matchId:String(m._id),prediction:'Double Chance',confidence:Math.max(55,68-i*4),risk:'low',analysis:'System fallback selection based on scheduled fixture availability; this is not a guarantee.'})); }
  await BetOfDay.deleteMany({date,status:{$in:['draft','approved']}});
  const docs=[]; for(const p of picks.slice(0,2)){ if(!matches.some(m=>String(m._id)===String(p.matchId))) continue; docs.push(await BetOfDay.findOneAndUpdate({date,matchId:p.matchId},{$set:{prediction:p.prediction,confidence:p.confidence,risk:p.risk,analysis:p.analysis,model,status:'draft'}},{upsert:true,new:true,setDefaultsOnInsert:true})); }
  return docs;
}
