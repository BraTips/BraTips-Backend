...
export async function sendEmail(to:string,subject:string,html:string,text?:string){
  try{await cloudflareSend(to,subject,html,text||textFromHtml(html));return true;}
  catch(e){console.error("Email send failed:",{to,subject,error:e});return false;}
}
...
export async function sendUserAccountStatusEmail(to:string,name:string,role:string,status:string){return sendEmail(to,"BraTipsters account update",shell("Account update",`<p style="font-size:16px;line-height:26px;margin:0 0 16px">Hi ${esc(name)},</p><p style="font-size:15px;line-height:25px;color:#475467">Your BraTipsters account has been updated.</p><p style="font-size:15px;line-height:25px;color:#475467">Role: <strong style="color:#172033">${esc(role)}</strong><br>Status: <strong style="color:#172033">${esc(status)}</strong></p><p style="font-size:15px;line-height:25px;color:#475467">Please sign in to BraTipsters to review your account.</p>`));}
export async function sendWithdrawalEmail(to:string,name:string,status:string,amount:number,currency="USD"){return sendEmail(to,"BraTipsters withdrawal update",shell("Withdrawal update",`<p style="font-size:16px;line-height:26px;margin:0 0 16px">Hi ${esc(name)},</p><p style="font-size:15px;line-height:25px;color:#475467">Your withdrawal request for <strong style="color:#172033">${esc(currency.toUpperCase())} ${amount.toFixed(2)}</strong> is now <strong style="color:#172033">${esc(status)}</strong>.</p><p style="font-size:15px;line-height:25px;color:#475467">Approved payouts are processed manually within 1–3 business days.</p>`));}
